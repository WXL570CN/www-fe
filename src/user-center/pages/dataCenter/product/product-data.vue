<template>
  <div class="module_right">
    <sub-title title="产品数据" class="data"></sub-title>
    <div class="firm-list">
      <ol>
        <li v-for="(item,index) in items" v-bind:key="item.id">
          <div class="list-info">
            <p>{{item.search_name}}</p>
            <span>{{item.create_time|formateUnixTime('YYYY-MM-DD')}}</span>
          </div>
          <div class="list-state">
            ...
            <div>
              <i class="revise"></i>
              <span v-on:click="redact(item.id)">编辑</span>
            </div>
            <!-- <div>
              <i class="download"></i>
              <span>下载</span>
            ...
            <div>
              <i class="delete"></i>
              <span v-on:click="delect(item.id,index)">删除</span>
            </div>
          </div>
        </li>
      </ol>
